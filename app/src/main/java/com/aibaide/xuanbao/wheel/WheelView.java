...
import com.sunshine.utils.GraphicUtil;
import com.sunshine.utils.ViewUtil;
...
public class WheelView extends View {
	...
	private static int[] SHADOWS_COLORS = new int[] { 0xffffff, 0x00ffffff, 0x00ffffff };
	...
	private int[] centerSelectGradientColors = new int[] { 0x20999999, 0x002ccddc, 0x20999999 };

	/** The center select stroke width. */
	private int centerSelectStrokeWidth = 1;

	/** The center select stroke color. */
	private int centerSelectStrokeColor = 0xffffff;
	...
	private int[] bottomGradientColors = new int[] { 0xffffff, 0x33ffffff, 0xffffff };
	// 轮子的背景 顶部的颜色
	/** The top gradient colors. */
	private int[] topGradientColors = new int[] { 0xffffff, 0xFFF, 0xffffff };

	/** The top stroke width. */
	private int topStrokeWidth = 1;

	/** The top stroke color. */
	private int topStrokeColor = 0xFFffffff;
	...
	private void initResourcesIfNecessary() {
		if (itemsPaint == null) {
			itemsPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG | Paint.SUBPIXEL_TEXT_FLAG);
		...
		if (labelPaint == null) {
			labelPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG | Paint.SUBPIXEL_TEXT_FLAG | Paint.DITHER_FLAG);
	...
	private int calculateLayoutWidth(int widthSize, int mode) {
		...
		if (maxLength > 0) {
			// 一个字符宽度
			float textWidth = (int) GraphicUtil.getStringWidth("0", labelPaint);
			// 不算lable的宽度
		...
		if (label != null && label.length() > 0) {
			labelWidth = (int) GraphicUtil.getStringWidth(label, labelPaint);
		}
	...
	private void createLayouts(int widthItems, int widthLabel) {
		...
		if (itemsLayout == null || itemsLayout.getWidth() > widthItems) {
			itemsLayout = new StaticLayout(buildText(isScrollingPerformed), itemsPaint, widthItems,
					widthLabel > 0 ? Layout.Alignment.ALIGN_OPPOSITE : Layout.Alignment.ALIGN_CENTER, 1, additionalItemHeight, false);
		} else {
			itemsLayout.increaseWidthTo(widthItems);
		}

		if (!isScrollingPerformed && (valueLayout == null || valueLayout.getWidth() > widthItems)) {
			String text = getAdapter() != null ? getAdapter().getItem(currentItem).toString() : null;
	...
	public void setValueTextSize(int textSize) {
		this.valueTextSize = ViewUtil.scale(mContext, textSize);
	...
	public void setLabelTextSize(int labelTextSize) {
		this.labelTextSize = ViewUtil.scale(mContext, labelTextSize);
	}
