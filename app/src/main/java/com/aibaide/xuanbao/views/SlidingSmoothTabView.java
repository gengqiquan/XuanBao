...
import com.aibaide.xuanbao.views.SlidingHorizontalScrollView.OnScrollListener;
 ...
 */
public class SlidingSmoothTabView extends LinearLayout {
	...
	/** tab的线性布局父. */
	private SlidingHorizontalScrollView mTabScrollView = null;
	 ...
	 *            the attrs
	 */
	public SlidingSmoothTabView(Context context, AttributeSet attrs) {
		super(context, attrs);
		...
		mTabScrollView = new SlidingHorizontalScrollView(context);
		...
		this.addView(mTabImg, new LayoutParams(
				LayoutParams.WRAP_CONTENT, tabSlidingHeight));

		// 内容的View的适配
		mViewPager = new ViewPager(context);
		// 手动创建的ViewPager,必须调用setId()方法设置一个id
		mViewPager.setId(R.id.SlidingSmoothTabView_viewpagerid);
		...
		mTabScrollView.setOnScrollListener(new OnScrollListener() {
			@Override
	...
	public void computeTabImg(int index) {
		...
		LayoutParams mParams = new LayoutParams(tabView.getMeasuredWidth(),
				tabSlidingHeight);
		...
		if (mSelectedTabIndex < index
				&& tabView.getRight() - scrollX > mTabScrollView.getWidth()) {
			int offsetX = 0;
			// 右边被遮挡
			if (index == mTabLayout.getChildCount() - 1) {
				offsetX = tabView.getRight() - mTabScrollView.getWidth()
						- scrollX;
				mTabScrollView.smoothScrollBy(offsetX, 0);
				scrollX = scrollX + offsetX;
				imageSlide(mTabImg, startX,
						mTabScrollView.getWidth() - tabView.getMeasuredWidth(),
						0, 0);
	 ...
	 *            the fragments
	 */
	public void addItemViews(List<String> tabTexts, List<GoodsTypeItemFragment> mFragments) {
