...
public class ViewPagerIndicator extends LinearLayout {
	 ...
	 * 三角形的宽度为单个Tab的1/6
	 */
	private static final float RADIO_TRIANGEL = 1.0f;
	/**
	 * 三角形的最大宽度
	 */
	 ...
	 * 标题正常时的颜色
	 */
	private int mColorDefult = R.color.white;
	/**
	 * 标题选中时的颜色
	 */
	private int mColorChecked = R.color.tab_check;
	...
	public ViewPagerIndicator(Context context, AttributeSet attrs) {
		...
		mPaint.setColor(getResources().getColor(mColorChecked));
	...
	public void setTabItemTitles(List<String> datas) {
		...
		if (datas != null && datas.size() > 0) {
			...
			for (int i = 0, l = mTabTitles.size(); i < l; i++) {
				TextView tv = new TextView(getContext());
				LayoutParams lp = new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
				lp.width = getScreenWidth() / mTabVisibleCount;
				tv.setGravity(Gravity.CENTER);
				tv.setTextColor(getResources().getColor(R.color.itry_list_goods_name));
				if (i == 0)
					tv.setTextColor(getResources().getColor(mColorChecked));
				tv.setText(mTabTitles.get(i));
	...
	protected void highLightTextView(int position) {
		...
		if (view instanceof TextView) {
			((TextView) view).setTextColor(getResources().getColor(mColorChecked));
		}
	...
	private void resetTextViewColor() {
		for (int i = 0; i < getChildCount(); i++) {
			...
			if (view instanceof TextView) {
				((TextView) view).setTextColor(getResources().getColor(R.color.itry_list_goods_name));
			}
		}
	}
	...
	private void initTriangle() {
		mPath = new Path();
		mTriangleHeight = DensityUtils.dp2px(getContext(), 2);
		mPath.moveTo(0, 0);
