...
import com.aibaide.xuanbao.wheel.NumericWheelAdapter;
import com.aibaide.xuanbao.wheel.StringWheelAdapter;
import com.aibaide.xuanbao.wheel.WheelView;
...
public class WheelUtil {
	...
	public static void initWheelDatePicker(final Context mContext,
			final getDataListerner listerner) {
		View view = LayoutInflater.from(mContext).inflate(
				R.layout.wheel_layout, null);
		final WheelView mWheelViewY = (WheelView) view
				.findViewById(R.id.wheelView1);
		final WheelView mWheelViewM = (WheelView) view
				.findViewById(R.id.wheelView2);
		final WheelView mWheelViewD = (WheelView) view
		...
		// 添加"年"监听
		WheelView.AbOnWheelChangedListener wheelListener_year = new WheelView.AbOnWheelChangedListener() {
		...
		// 添加"月"监听
		WheelView.AbOnWheelChangedListener wheelListener_month = new WheelView.AbOnWheelChangedListener() {
		...
		builder.setNeutralButton("确认", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				listerner.getData(year,month,day);
			}
		});
	...
	public interface getDataListerner {
		void getData(String year, String month, String day);
	}
	...
	public static void initWheelTimePicker(final Activity activity,
			final TextView mText, final WheelView mWheelViewMD,
		...
		String val = StringUtil.dateTimeFormat(defaultYear + "-" + defaultMonth
		...
		// 月日
		mWheelViewMD.setAdapter(new StringWheelAdapter(textDMList, StringUtil
				.strLength("12月" + " " + "12日")));
		...
		okBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Calendar calendar = Calendar.getInstance();
				int second = calendar.get(Calendar.SECOND);
				String val = StringUtil.dateTimeFormat(dmStr + " " + index2
						+ ":" + index3 + ":" + second);
				mText.setText(val);
			}
	...
	public static void initWheelTimePicker2(final Activity activity,
			final TextView mText, final WheelView mWheelViewHH,
			final WheelView mWheelViewMM, Button okBtn, Button cancelBtn,
		...
		String val = StringUtil.dateTimeFormat(defaultHour + ":"
				+ defaultMinute);
		mText.setText(val);
		...
		okBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				int index2 = mWheelViewHH.getCurrentItem();
				int index3 = mWheelViewMM.getCurrentItem();
				String val = StringUtil.dateTimeFormat(index2 + ":" + index3);
				mText.setText(val);
			}
